...
impl Terminal {
    ...
    pub async fn write_raw(&self, data: &[u8]) -> Result<()> {
        debug!("write_raw: Writing {} bytes to terminal {}", data.len(), self.id);
    ...
    pub async fn write_line(&self, text: &str) -> Result<()> {
        debug!("write_line: Writing line '{}' to terminal {}", text, self.id);
    ...
    pub async fn send_control(&self, c: char) -> Result<()> {
        ...
        debug!("Sent control character Ctrl-{} to terminal {}", c.to_uppercase(), self.id);
        Ok(())
    }
    ...
    async fn read_available(&self) -> Result<Vec<u8>> {
        ...
        debug!("read_available: Starting read operation for terminal {}", self.id);
        ...
        loop {
            match tokio::time::timeout(Duration::from_millis(10), reader.read(&mut buffer)).await {
                ...
                Ok(Ok(n)) => {
                    ...
                    debug!("read_available: Read {} bytes from PTY for terminal {}", n, self.id);
                ...
                Ok(Err(e)) => {
                    debug!("read_available: PTY read error for terminal {}: {}", self.id, e);
                    return Err(e.into());
                }
                Err(_) => {
                    // Timeout - no more data available
                    debug!("read_available: No more data available for terminal {}", self.id);
                    break;
                }
            }
        }
    ...
    pub async fn execute_command(&self, command: &str, timeout_ms: u64) -> Result<String> {
        debug!("Execute command '{}' with timeout {}ms", command, timeout_ms);
        ...
        while start.elapsed() < Duration::from_millis(timeout_ms) {
            match self.read_available().await {
                Ok(output) => {
                    ...
                    } else if last_data_time.elapsed() > Duration::from_millis(500) && !all_output.is_empty() {
    ...
    pub async fn read_streaming(&self) -> Result<String> {
        ...
        debug!("read_streaming: Returning {} chars for terminal {}", result.len(), self.id);
        
        Ok(result)
    }
    
    /// Wait for a specific prompt to appear in the output
    pub async fn wait_for_prompt(&self, prompts: &[&str], timeout_ms: u64, include_output: bool) -> Result<(Option<String>, String)> {
        debug!("wait_for_prompt: Waiting for prompts {:?} with timeout {}ms", prompts, timeout_ms);
            ...
            if !output.is_empty() {
                ...
                for prompt in prompts {
                    if output_str.contains(prompt) {
                        ...
                        debug!("wait_for_prompt: Found prompt '{}' after {}ms", prompt, start.elapsed().as_millis());
    ...
    pub async fn read_until_stable(&self, stability_ms: u64, max_timeout_ms: u64, return_partial_on_timeout: bool) -> Result<String> {
        debug!("read_until_stable: stability_ms={}, max_timeout_ms={}", stability_ms, max_timeout_ms);
        ...
        while start.elapsed() < Duration::from_millis(max_timeout_ms) {
            ...
            if !output.is_empty() {
                ...
                debug!("read_until_stable: Got {} bytes, total: {}", output.len(), collected_output.len());
            } else if last_data_time.elapsed() >= Duration::from_millis(stability_ms) {
                // Output has been stable for the required duration
                debug!("read_until_stable: Output stable for {}ms, returning", stability_ms);
        ...
        if return_partial_on_timeout || !collected_output.is_empty() {
            ...
            Err(TerminalError::Timeout(format!("read_until_stable timed out after {}ms", max_timeout_ms)))
        }
    }
    ...
    pub async fn set_debug_mode(&self, enabled: bool) -> Result<()> {
        *self.debug_mode.lock().await = enabled;
        debug!("Debug mode {} for terminal {}", if enabled { "enabled" } else { "disabled" }, self.id);
        Ok(())
    }
    ...
    pub async fn wait_for_ready(&self, timeout_ms: u64) -> Result<()> {
        debug!("Waiting for session to be ready with timeout {}ms", timeout_ms);
        ...
        match tokio::time::timeout(Duration::from_millis(timeout_ms), async {
            ...
            loop {
                ...
                if !output.is_empty() {
                    ...
                    stable_count = 0;
                    debug!("Got {} bytes during wait_for_ready, total: {}", output.len(), total_output);
                    
                    // Check if we got a shell prompt (common patterns)
                    let output_str = String::from_utf8_lossy(&output);
                    if output_str.contains("$") || output_str.contains("#") || output_str.contains(">") {
                    ...
                    if elapsed >= Duration::from_millis(DEBOUNCE_MS) {
                        ...
                        if stable_count >= STABLE_THRESHOLD {
                            debug!("Session ready after {} stable checks ({}ms debounce)", stable_count, DEBOUNCE_MS);
...
impl TerminalManager {
    ...
    pub async fn read_streaming_with_timeout(&self, name: &str, timeout_ms: u64) -> Result<String> {
        debug!("read_streaming_with_timeout: Called for session '{}' with timeout {}ms", name, timeout_ms);
        ...
        if let Some(terminal) = self.get_terminal(name).await {
            ...
            match tokio::time::timeout(Duration::from_millis(timeout_ms), terminal.read_streaming()).await {
                Ok(Ok(output)) => {
                    debug!("read_streaming_with_timeout: Got {} chars from session '{}'", output.len(), name);
                    Ok(output)
                },
                Ok(Err(e)) => {
                    debug!("read_streaming_with_timeout: Error reading from session '{}': {}", name, e);
                    Err(e)
                },
                Err(_) => {
                    debug!("read_streaming_with_timeout: Timeout after {}ms for session '{}'", timeout_ms, name);
